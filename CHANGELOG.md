...
- A logo for the package under `docs/assets/logo/`: `logo.svg` (horizontal lock-up, now at the
  ...
  whose strokes survive 16 px), `logo-og.svg` (1200×630 social card) and `logo-social.svg`
  (1280×640, the 2:1 GitHub renders repository previews at), plus rasterised `png/` sizes —
  favicons 16/32/48, app icons 64…1024, `apple-touch-icon.png`, README lock-up at 1x and 2x,
  `og-image.png`, `social-preview.png` — and `docs/favicon.ico`, bundling 16/32/48 for hosts
  that look for it at the docs root
